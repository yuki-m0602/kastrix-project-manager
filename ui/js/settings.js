...
async function renderSettings() {
  ...
  let dirs = [], openaiStatus = false, anthropicStatus = false, openrouterStatus = false, defaultProvider = 'openai', syncMode = SYNC_MODE_AUTO, teamReady = false;
  ...
  try {
    ...
    [dirs, openaiStatus, anthropicStatus, openrouterStatus, defaultProvider, aiModelOpenai, aiModelAnthropic, aiModelOpenrouter, syncMode, teamReady] = loaded;
  } catch (e) {
  ...
  container.innerHTML = `
    ...
    <!-- Team (サイドバーのTeamビューを優先) -->
    <section class="bg-[#161b22] border border-[#30363d] rounded-2xl p-4 sm:p-6">
      <h2 class="text-sm font-bold text-white mb-1">チーム</h2>
      <p class="text-[10px] text-[#8b949e] mb-4">チーム機能はサイドバーの「Team」メニューから利用できます。</p>
      <button onclick="setActiveMenu('team')" class="h-8 px-4 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-xs font-bold text-white flex items-center gap-2">
